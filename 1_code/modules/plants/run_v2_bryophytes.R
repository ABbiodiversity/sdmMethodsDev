# ---
# title: Run v2 Bryophyte Modelling Pipeline
# author: Brendan Casey
# created: 2026-09-05
# inputs:
#   scripts, in 0_data/v2_scripts/plants/:
#     - 02a_hierarchical-models-bryophytes.R
#     - 03a_model-validation-bryophytes.R
...
#     - 3_output/validation/bryophyte-models-validation.Rdata
#   in this repository:
...
#   - 03a validates with protocol.flag = TRUE, unlike 03c and
#     03d.
#   - The v2 scripts hard-code 14 cores and 100 bootstraps and
#     register no foreach backend, so %dopar% runs sequentially.
#   - 03a's header misnames its output byrophyte-...Rdata; the
#     code writes bryophyte-...Rdata.
...
if (!exists("run_02a", inherits = FALSE)) {
  run_02a <- TRUE # 02a_hierarchical-models-bryophytes.R
}
if (!exists("run_03a", inherits = FALSE)) {
  run_03a <- TRUE # 03a_model-validation-bryophytes.R
}
...
if (run_03a) {
  # 03a reads the models written by 02a. When 02a runs in the same
...
if (run_02a) {
  run_step(
    label = "02a: Hierarchical models (bryophytes)",
    script = "02a_hierarchical-models-bryophytes.R",
...
if (run_03a) {
  run_step(
    label = "03a: Model validation (bryophytes)",
    script = "03a_model-validation-bryophytes.R",
    code_dir = v2_code_dir,
    run_dir = v2_root,
    log_dir = log_path,
    protect = runner_objects
