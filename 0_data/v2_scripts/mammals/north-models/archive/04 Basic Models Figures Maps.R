...
g_drive <- "G:/Shared drives/ABMI Camera Mammals/Results/Habitat Models/2024/analysis-north/"
...
km2file.out <- "G:/Shared drives/ABMI Mammals/Data Habitat Models/km2-grid-north_current-backfilled_processed_2026-05-015.Rdata"    # File with processed km2 grid files - processed in separate script.  Despite the variable name, this is an input here.
...
dataset.out<-"G:/Shared drives/ABMI Mammals/Data/Habitat Models/R Dataset SpTable for ABMI North mammal coefficients 2024.RData"
load(dataset.out)

# Add 'Climate' to pm
# Note: Might not need to do this anymore.
# pm <- read_csv(paste0("G:Shared drives/ABMI Camera Mammals/data/lookup/veghf/prediction-matrix_north.csv"))

SpTable.w<-SpTable.w[SpTable.w!="Black BearWinter"]  # Bears have enough occurrences in winter to meet criterion, but they are just flukey encounters - don't run winter models
...
# I'm going to remove the following species (unreliable data)
SpTable <- SpTable[SpTable != "Deer"]
SpTable <- SpTable[SpTable != "Red Squirrel"]
SpTable <- SpTable[SpTable != "Weasels and Ermine"]
SpTable <- SpTable[SpTable != "Porcupine"]
SpTable <- SpTable[SpTable != "White-tailed Jack Rabbit"]
SpTable <- SpTable[SpTable != "Striped Skunk"]
...
# Changing sc.option to 3 to prevent from running that section
sc.option <- rep(3, length(SpTable))
...
km2.sc<-data.frame(km2[,c("Intercept","Lat","Long","TrueLat","AHM","FFP","MAP","MAT","MCMT","MWMT","NSR1Parkland","NSR1DryMixedwood","NSR1CentralMixedwood","NSR1Foothills","NSR1North","NSR1Shield","NSR1Mountain")],
                   Lat2=km2$Lat^2,Lat3=km2$Lat^3,Long2=km2$Long^2,LatLong=km2$Lat*km2$Long,Lat2Long2=km2$Lat^2*km2$Long^2,LongMAT=km2$Long*km2$MAT,MATAHM=km2$MAT*km2$AHM,MAPFFP=km2$MAP*km2$FFP,
                   MAT2=km2$MAT*(km2$MAT+10),MWMT2=km2$MWMT^2)
...
jpeg(file="G:/Shared drives/ABMI Camera Mammals/Results/Habitat Models/2024/analysis-north/maps/Spatial projection area NORTH.jpg",height=600,width=400)
...
vnames.b<-c("Climate", "SpruceR","Spruce1","Spruce2","Spruce3","Spruce4","Spruce5","Spruce6","Spruce7","Spruce8","PineR","Pine1","Pine2","Pine3","Pine4","Pine5","Pine6","Pine7","Pine8","DecidR","Decid1","Decid2","Decid3","Decid4","Decid5","Decid6","Decid7","Decid8",
            ...
            "TreedFen","TreedSwamp","Shrub","GrassHerb","GraminoidFen","Marsh","ShrubbyBog","ShrubbyFen","ShrubbySwamp")  # Variables to use in reference predictions

# .pa used for presence/absence components, .agp for abundance-given-presence, .sc for space/climate, .s for summer, .w for winter
vnames.pa<-c(
             "Climate",
             "SpruceR","Spruce1","Spruce2","Spruce3","Spruce4","Spruce5","Spruce6","Spruce7","Spruce8","PineR","Pine1","Pine2","Pine3","Pine4","Pine5","Pine6","Pine7","Pine8","DecidR","Decid1","Decid2","Decid3","Decid4","Decid5","Decid6","Decid7","Decid8",
             "MixedwoodR","Mixedwood1","Mixedwood2","Mixedwood3","Mixedwood4","Mixedwood5","Mixedwood6","Mixedwood7","Mixedwood8","TreedBogR","TreedBog1","TreedBog2","TreedBog3","TreedBog4","TreedBog5","TreedBog6","TreedBog7","TreedBog8",
             ...
             "Crop","TameP","RoughP","Urban","Rural","Industrial","Well","EnSoftLin","EnSeismic","TrSoftLin","HardLin")  # Names of coefficients
...
# Removing Climate for now.
vnames.agp<-c(
              "Climate",
              "Spruce","Pine","Decid","Mixedwood","TreedBog",
              "TreedFen","TreedSwamp","Shrub","GrassHerb","GraminoidFen","Marsh","ShrubbyBog","ShrubbyFen","ShrubbySwamp",
              "CCSpruceR","CCSpruce1","CCSpruce2","CCSpruce3","CCSpruce4","CCPineR","CCPine1","CCPine2","CCPine3","CCPine4","CCDecidR","CCDecid1","CCDecid2","CCDecid3","CCDecid4","CCMixedwoodR","CCMixedwood1","CCMixedwood2","CCMixedwood3","CCMixedwood4",
              "Crop","TameP","RoughP","Urban","Rural","Industrial","Well","EnSoftLin","EnSeismic","TrSoftLin","HardLin")
...
Coef.pa.all<-Coef.pa.lci.all<-Coef.pa.uci.all<-Coef.pa.se.all<-Coef.mean.all<-Coef.lci.all<-Coef.uci.all<-
  array(0, c(length(SpTable), length(vnames.pa)))  # avg of Summer and Winter

colnames(Coef.pa.all)    <-
  colnames(Coef.pa.lci.all) <-
  colnames(Coef.pa.uci.all) <-
  colnames(Coef.pa.se.all)  <-
  colnames(Coef.mean.all)   <-
  colnames(Coef.lci.all)    <-
  colnames(Coef.uci.all)    <- vnames.pa

rownames(Coef.pa.all)    <-
  rownames(Coef.pa.lci.all) <-
  rownames(Coef.pa.uci.all) <-
  rownames(Coef.pa.se.all)  <-
  rownames(Coef.mean.all)   <-
  rownames(Coef.lci.all)    <-
  rownames(Coef.uci.all)    <- SpTable
...
vnames.sc<-c("Intercept","Lat","Long","LatLong","Lat2","Lat3","Long2","Lat2Long2","AHM","MAT","FFP","MAP","MAPFFP","MATAHM","MWMT","MCMT","MWMT2","MAT2","LongMAT","NSR1Parkland","NSR1DryMixedwood","NSR1CentralMixedwood","NSR1Foothills","NSR1North","NSR1Shield","NSR1Mountain")
...
# New - append new climate predictions

climate_pred <- read_csv("G:/Shared drives/ABMI Mammals/Results/Habitat Modeling/2024/Climate/Predictions/All Species Climate Predictions.csv")

clim <- climate_pred |>
  pivot_longer(Badger:`Woodland Caribou`, names_to = "Species", values_to = "Climate")
...
# Remove WetlandMargin sites
d <- d |>
  filter(!WetlandMargin == "1")

# Remove WetlandMargin from pm
pm <- pm |>
  select(-WetlandMargin) |>
  filter(!VegType == "WetlandMargin")
...
for (sp in 1:length(SpTable)) {
  ...
  # Filter climate preds
  c <- clim |>
    filter(Species == SpTable[sp]) |>
    select(1, 3)
  ...
  for (seas in 1:2) {  # 1=Summer, 2=Winter
    ...
    if ( (seas==1 & SpSeas %in% SpTable.s==TRUE) | (seas==2 & SpSeas%in%SpTable.w==TRUE) ) {  # Only run if that species is in the species table for that season
      ...
      # Join predictions from the Climate modeling in the previous script
      d.sp <- d.sp |>
        inner_join(c, by = "location_project") |>
        filter(!is.na(Climate),
               !is.na(Count)) |>
        select(location_project, Climate, everything())

      # Re-do SeasDays ...
      if (seas==1) {  # Set season-specific variables
        SpSeas<-paste(SpTable[sp],"Summer",sep="")
        SeasDays<-d.sp$SummerDays
        SeasWt<-d.sp$wt.s  # Set up in first script
      }
      if (seas==2) {
        SpSeas<-paste(SpTable[sp],"Winter",sep="")
        SeasDays<-d.sp$WinterDays
        SeasWt<-d.sp$wt.w  # Set up in first script
      }
      ...
      m.pa[[3]]<-try(glm(pCount~Decid+Mixedwood+Pine+Spruce+TreedBog+TreedFen+TreedSwamp+GrassHerb+Shrub+Marsh+ShrubbySwamp+ShrubbyBogFen+CCDecidMixed+CCPine+CCSpruce+EnSoftLin+EnSeismic+TrSoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[4]]<-try(glm(pCount~Decid+Mixedwood+Pine+Spruce+TreedBog+TreedFen+TreedSwamp+GrassHerb+Shrub+Marsh+ShrubbySwamp+ShrubbyBogFen+CCDecidMixed+CCPine+CCSpruce+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[5]]<-try(glm(pCount~Decid+Mixedwood+Pine+Spruce+TreedBog+TreedFen+TreedSwamp+GrassHerb+Shrub+Marsh+ShrubbySwamp+ShrubbyBogFen+CCAll+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien.  Weak data for cutblocks
      m.pa[[6]]<-try(glm(pCount~Decid+Mixedwood+Pine+Spruce+TreedBogFen+TreedSwamp+GrassHerb+Shrub+OpenWet+CCAll+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien.
      m.pa[[7]]<-try(glm(pCount~Decid+Mixedwood+Pine+Spruce+TreedWet+GrassShrub+OpenWet+CCDecidR+CCDecid1+CCDecid2+CCMixedwoodR+CCMixedwood1+CCMixedwood2+CCPineR+CCPine1+CCSpruceR+CCSpruce1+CCSpruce2+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[8]]<-try(glm(pCount~Decid+Mixedwood+Pine+Spruce+TreedWet+GrassShrub+OpenWet+CCDecidMixed+CCConif+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[9]]<-try(glm(pCount~DecidMixed+UpCon+TreedWet+GrassShrub+OpenWet+CCDecidMixed+CCConif+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[10]]<-try(glm(pCount~DecidMixed+UpCon+TreedWet+GrassShrub+OpenWet+CCAll+SoftLin+TameP+RoughP+Well+RurUrbInd+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is crop
      m.pa[[11]]<-try(glm(pCount~DecidMixed+UpCon+TreedWet+GrassShrub+OpenWet+CCAll+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[12]]<-try(glm(pCount~DecidMixed+UpCon+TreedWet+GrassShrub+OpenWet+Succ+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[13]]<-try(glm(pCount~Upland+Lowland+CCAll+SoftLin+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[14]]<-try(glm(pCount~TreedAll+OpenAll+Succ+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[15]]<-try(glm(pCount~UplandForest+Lowland+GrassShrub+Succ+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[16]]<-try(glm(pCount~Upland+Lowland+Succ+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # Intercept is alien
      m.pa[[17]]<-try(glm(pCount~Boreal+GrassShrub+Succ+SeasDays+Climate,family="binomial",data=d.sp,weights=SeasWt))  # "Boreal" is everything native except GrassShrub. Intercept is alien
      ...
      for (i in 1:nModels.pa) {
        ...
        x <- x[!names(x) %in% c("Climate", "SeasDays")]

        #if (min(x)>3 & nrow(d.p)-sum(x[-length(x)]))  {
        # Only use the model if each type is represented by >3 presence records (incl. intercept)
        # Okay, we're going to change this to needing at least 10 presence records.
        if (min(x) > 9 & nrow(d.p) - sum(x)) {

          j<-j+1

          # Original - with Climate.
          ...
          # Removing Climate from all these formulas
          form_terms <- attr(m.pa[[i]]$terms, "term.labels")
          form_terms <- setdiff(form_terms, c("Climate"))
          new_formula <- as.formula(paste("pCount ~", paste(form_terms, collapse = " + ")))

          # To new_formula, add " + offset(Climate)
          # This is the Climate prediction in the probability space.

          m.agp[[j]] <- glm(new_formula, data = d.p, family = gaussian(link = "log"))
      ...
      # Note: Removing Climate here now.
      # m.agp[[j+1]]<-glm(pCount ~ SeasDays + Climate, data=d.p,family=gaussian(link="log"))  # And add a null
      ...
      # These are the intercepts - the veg+HF type that is not included in the model terms (to avoid collinearity).
      # Change this if models change.
      Intercept <- c("Crop","Crop",rep("Alien",7),"Crop",rep("Alien",7))[best.model.pa]

      # Gathering the terms from the best model, putting the Intercept back.
      ...
      for (i in 1:length(terms.pa)) {
        ...
        p<-predict(m.pa[[best.model.pa]],newdata=data.frame(SeasDays=100, Climate = 0, pm1),se.fit=T)
        Coef1.pa[i]<-plogis(p$fit)  # Ordinal scale
        Coef1.pa.se[i]<-p$se.fit  # logit scale
      }
      # Adjust so that mean prediction at standardized SeasDays across all qualifying sites = mean observed count.
      # This is to compensate for inaccuracies due to fitting SeasDays coefficients
      pCount<-sign(d.sp$Count)/ifelse(d.sp$Lured=="Yes",lure.pa[seas,sp],1)  # Standardize all to no-lure
      pCount<-pCount/max(pCount)  # This is in case the lure effect is <1

      # Do we want to remove Climate from here ... ?
      # No, we actually want to manually grab the Climate coefficient and replace it.
      climate_estimate <- plogis(summary(m.pa[[best.model.pa]])$coefficients["Climate", "Estimate"])

      Coef1.pa["Climate"] <- climate_estimate

      # Prediction for each data point, but at standardized days
      # Should not be Climate = 0.
      # p.adj<-predict(m.pa[[best.model.pa]],newdata=data.frame(SeasDays=100, Climate = 0, d.sp))
      p.adj<-predict(m.pa[[best.model.pa]],newdata=data.frame(SeasDays=100, d.sp))

      # Adjustment made on logit scale.  Var doesn't change on logit scale(?).
      # The adjustment = qlogis(mean(pCount))-qlogis(mean(plogis(p.adj))) should be small
      Coef1.pa<-plogis(qlogis(Coef1.pa)+qlogis(mean(pCount))-qlogis(mean(plogis(p.adj))))
      ...
      # NEW - Remove "Climate"
      # terms.pa1 <- terms.pa1[terms.pa1 != "Climate"]
      ...
      # Remove Climate = 0 in this new test.
      # Or, does it really matter?
      p<-predict(m.agp[[i]],newdata=data.frame(pm,SeasDays=100, Climate = 0),se.fit=T)  # For each type
      ...
      # Prediction for each data point, but at standardized days
      # Don't want Climate = 0 here either.
      # p.adj<-predict(m.agp[[i]],newdata=data.frame(SeasDays=100, Climate = 0, d.sp))
      p.adj<-predict(m.agp[[i]],newdata=data.frame(SeasDays=100, d.sp))

      # Pull intercept and Climate coefficient separately (from m.agp[[i]])
      #climate_estimate <- summary(m.agp[[best.model.agp]])$coefficients["Climate", "Estimate"]
      #names(climate_estimate) <- "Climate"

      # Var doesn't change on log scale
      tTypeMean.agp<-log(exp(tTypeMean.agp)+mean(pCount)-mean(exp(p.adj)))
      ...
      tTypeMean.agp <- c(climate_estimate, tTypeMean.agp)

      # Then average those for each broader group included in the best pa model

      for (i in 1:length(terms.pa1)) {

        if (terms.pa1[i] == "Climate") {

           Coef1.agp["Climate"] <- tTypeMean.agp["Climate"]
      ...
      # Get all coefficient names from the Coef.agp array's 3rd dimension
      coef_names <- dimnames(Coef.agp)[[3]]

      # Match those to Coef1.agp vector
      vals <- tTypeMean.agp[coef_names]

      # Apply exp() to everything except "Climate" - ordinal scale.
      vals[names(vals) != "Climate"] <- exp(vals[names(vals) != "Climate"])

      # Assign to Coef.agp matrix
      Coef.agp[seas, sp, ] <- vals
      ...
      # Post-hoc smoothing of age relationships, to deal with extreme values due to linear relationships on log
      # (=lower part of logit) scale
      ...
      # Remember to make sure Climate is 1 (watch out for being backtransformed)
      ...
      for (i in 1:length(terms.pa1)) {
        ...
        # If climate, do the manual merging
        if(terms.pa1[i] == "Climate") {

          Coef.pa[seas,sp,"Climate"]<-Coef1.pa["Climate"]  # Fill in all ages with value for that (broad) stand type
          Coef.pa.se[seas,sp,"Climate"]<-Coef1.pa.se["Climate"]  # Fill in all ages with value for that (broad) stand type
          Coef.mean[seas,sp,"Climate"]<-Coef.pa[seas,sp,"Climate"]*Coef.agp[seas,sp,"Climate"]  # Only the one value for agp
          Coef.lci[seas,sp,"Climate"]<-plogis(qlogis(Coef.pa[seas,sp,"Climate"])-Coef.pa.se[seas,sp,"Climate"]*1.28) * exp(log(Coef.agp[seas,sp,"Climate"])-Coef.agp.se[seas,sp,"Climate"]*1.28)  # Using 10% intervals for each to multiply to 5% intervals (assuming independence - checked empirically)
          Coef.uci[seas,sp,"Climate"]<-plogis(qlogis(Coef.pa[seas,sp,"Climate"])+Coef.pa.se[seas,sp,"Climate"]*1.28) * exp(log(Coef.agp[seas,sp,"Climate"])+Coef.agp.se[seas,sp,"Climate"]*1.28)  # Using 10% intervals for each to multiply to 5% intervals (assuming independence - checked empirically)

          next()

        }

        # The fine veg+HF types that are covered by the (potentially broader) variable included in the best model
        j<-as.character(pm$VegType[pm[,terms.pa1[i]]==1])
      ...
      # No Climate information in the agp model.
      Coef1.agp["Climate"] <- 0

      # Prediction of presence/absence at each site - ordinal scale
      d.sp$t.p.pa<-colSums(Coef1.pa[terms.pa1]*t(d.sp[,terms.pa1]))
      # Prediction of abundance|presence at each site - log scale
      d.sp$t.p.agp<-colSums(Coef1.agp[terms.pa1]*t(d.sp[,terms.pa1]))
      # Prediction (ordinal scale) of total abundance at each site
      d.sp$p.ta<-d.sp$t.p.pa*exp(d.sp$t.p.agp)
      ...
      # mean(pCount) should be very to mean(d.sp$p.ta)
  ...
  for (i in 1:length(terms.pa1)) {
    if (age.flag[terms.pa1[i]]==0 | is.na(age.flag[terms.pa1[i]])) {  # Variable without age info
      ...
      #y<-c(y,Coef.pa.all[seas,sp,veg.to.use])
      #y.lci<-c(y.lci,Coef.pa.lci.all[seas,sp,veg.to.use])
      #y.uci<-c(y.uci,Coef.pa.uci.all[seas,sp,veg.to.use])
  ...
  fname<-paste(fname.fig,"NEW PA Veg+HF figure best model ",SpTable[sp]," ",SeasName[seas],".png",sep="")
  ...
  # 5.1. Combine Summer and winter coefficients
   ...
   if (paste(SpTable[sp],"Summer",sep="") %in% SpTable.s  & paste(SpTable[sp],"Winter",sep="") %in% SpTable.w) {
     ...
     Coef.pa.se.all[sp,] <- (Coef.pa.se[1,sp,] + Coef.pa.se[2,sp,]) / 2
  ...
  vnames.agp1 <- vnames.agp[vnames.agp != "Climate"]

  for (i in 1:length(vnames.agp1)) {  # vnames.agp doesn't have age classes for natural stands
    if (vnames.agp1[i] %in% c("Spruce","Pine","Decid","Mixedwood","TreedBog") == FALSE) {  # Variable without age info
      j<-match(vnames.agp1[i],fp$Class)
      x<-c(x,fp$x[j])
      veg.to.use<-as.character(pm$VegType[pm[,vnames.agp1[i]]==1])[1]  # To use Coef.mean, find the first fine veg+HF types that is included in the (potentially broader) variable being plotted
      if (veg.to.use %in% c("Spruce","Pine","Decid","Mixedwood","TreedBog")) veg.to.use<-paste(veg.to.use,"R",sep="")  # If no age relationship, use the first value for that fine stand type (don't add the R for types that don't have age classes at all)
      ...
      col1<-c(col1,fp$col1[j])
      w<-c(w,fp$width[j])
      class<-c(class,vnames.agp1[i])
      space1<-c(space1,fp$spaceafter[j])
    } else {  # Variable with age info
      j<-match(paste(vnames.agp1[i],"R",sep=""),fp$Class):match(paste(vnames.agp1[i],"8",sep=""),fp$Class)
      x<-c(x,fp$x[j])
      veg.to.use<-as.character(pm$VegType[pm[,vnames.agp1[i]]==1])[1]  # To use Coef.mean, find the first fine veg+HF types that is included in the (potentially broader) variable being plotted
      veg.to.use<-paste(veg.to.use,c("R","1","2","3","4","5","6","7","8"),sep="")  # If age relationship, use each value for that fine stand type
      y<-c(y,Coef.mean.all[sp,veg.to.use])
  ...
  space<-c(1,x[-1]-x[-length(x)])-0.99  # The spacing between bars
  density<-ifelse(substr(class,1,2)=="CC",50,NA)
  fname<-paste(fname.fig,"NEW PA Veg+HF figure best model ",SpTable[sp],".png",sep="")
  png(file=fname,width=ifelse(length(y)>5,1500,1000),height=700)
  par(mai=c(1.9,1,0.2,0.3))
  x1<-barplot(y,space=space,width=w,border="white",col="grey30",ylim=c(0,ymax),yaxt="n",
              ylab="Relative abundance",col.lab="grey50",cex.lab=1.2,axisnames=F)[,1]  # To get strips on CC bars
  abline(h=pretty(c(0,ymax)),col="grey80")
  x1<-barplot(y,space=space,width=w,border="white",col="grey30",ylim=c(0,ymax),yaxt="n",ylab="Relative abundance",col.lab="grey50",cex.lab=1.2,axisnames=F,add=TRUE)[,1]  # To get strips on CC bars, and to put bars in front of horizontal axis lines
  x1<-barplot(y,space=space,width=w,border="white",density=density,col=col1,ylim=c(0,ymax),yaxt="n",ylab="Relative abundance",col.lab="grey50",cex.lab=1.2,axisnames=F,add=TRUE)[,1]
  axis(side=2,tck=0.02,cex.axis=0.9,col.axis="grey50",col.ticks="grey50",las=2,at=pretty(c(0,ymax)))
...
Coef.pa.se.all  <- cbind(Coef.pa.se.all,  rep(0, nrow(Coef.pa.se.all)),  rep(0, nrow(Coef.pa.se.all)))  # SEs for bare and water
...
colnames(Coef.pa.se.all)[(ncol(Coef.pa.se.all)-1):ncol(Coef.pa.se.all)] <- c("Bare","Water")  # SEs set to 0

fname.sumout <- "G:/Shared drives/ABMI Camera Mammals/Results/Habitat Models/2024/analysis-north/coefficient-tables/2024 North Mammal Coefficients_"

# Save
fname<-paste0(fname.sumout,"2025-12-10.Rdata")
save(file=fname,
     Coef.mean.all,
     #Coef.official,Coef.official.lci,Coef.official.uci,
     #Res.coef.official,
     Coef.pa.all,
     Coef.pa.se.all)  # Save to compile later with South results
...
# Therefore, the standard space/climate modeling excludes those lines - they are commented out - , so the largest dataset can be used.
  ...
  d2<-cbind(d1,d[,c("Lat","TrueLat","Long","PET","AHM","MAT","FFP","MAP","MWMT","MCMT","NSR1")])  # For space/climate modeling only (no 150m options)
  d2<-d2[!is.na(rowSums(d2[,c("Lat","TrueLat","Long","PET","AHM","MAT","FFP","MAP","MWMT","MCMT")])),]  # For space/climate modeling only (no 150m veg).  For AIC, only use records with all explanatory variables
  #	d2<-cbind(d1,d[,c("Lat","TrueLat","Long","PET","AHM","MAT","FFP","MAP","MWMT","MCMT","NSR1","xAlien","xSucc","xNonlin","xLin","xTHF","xDecid","xUpCon","xLowTreed","xLowOpen","xUpOpen","xOld","xYoung","xUpland","xLowland")])
  #	d2<-d2[!is.na(rowSums(d2[,c("Lat","TrueLat","Long","PET","AHM","MAT","FFP","MAP","MWMT","MCMT","xAlien","xSucc","xNonlin","xLin","xTHF","xDecid","xUpCon","xLowTreed","xLowOpen","xUpOpen","xOld","xYoung")])),]  # For AIC, only use records with all explanatory variables
...
# Okay, can we use this code to make prediction maps?
...
# Save
fname<-paste(fname.sumout,"OFFICIAL coefficients_2025-07-08.Rdata")
save(file=fname,
     Coef.mean.all,
     #Coef.official,Coef.official.lci,Coef.official.uci,
     #Res.coef.official,
     Coef.pa.all)  # Save to compile later with South results
